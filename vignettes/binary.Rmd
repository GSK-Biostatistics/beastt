...
**Note: if reading internal data directly into `calc_post_beta` instead of a propensity score object, be sure to include only the arm of interest (e.g., the internal control arm if creating a posterior distribution for $\theta_C$).**

```{r}
post <- calc_post_beta(filter(int_binary_df, trt==0),
                       response = y,
                       prior = pwr_prior)
plot_dist(post)
```

If we want to robustify our power prior for $\theta_C$, we can add a vague component to the power prior distribution we previously created to construct a robust mixture prior. In general, we can define our prior to be a mixture distribution with an arbitrary number of beta components, and see that this prior has features similar to the components.

```{r}
mix_prior <- dist_mixture(pwr_prior,
                          dist_beta(0.5, 0.5),
                          weights = c(0.5, 0.5))
vague <- dist_beta(0.5, 0.5)
plot_dist("Vague Prior" = vague, 
          "Robust Mixture Prior" = mix_prior,
          "Power Prior" = pwr_prior)
```

We can pass the resulting robust mixture prior to `calc_post_beta`, and see that the resulting posterior distribution will also be a mixture of beta components.

```{r}
post_mixed <- calc_post_beta(filter(int_binary_df, trt==0),
                             response = y,
                             prior = mix_prior)
